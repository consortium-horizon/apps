...
/**
 * IMPORTANT - Modified version for Vanilla 2.1, production version. NOT
 * COMPATIBLE with Vanilla 2.0. It has not been tested with any other 2.1
 * version.
 * This file has been modified and distributed with Post Scheduler plugin. The
 * following changes have been made:
 * - Added firing of event BeforeProcessingActivityNotifications in ActivityModel::Save()
 * - Added firing of event AfterActivityQuery in ActivityModel::GetWhere()
 *
 * NOTES
 ...
 * contact the author, Diego Zanella <diego@aelia.co>.
 */
   ...
   public function Save($Data, $Preference = FALSE, $Options = array()) {
      ...
      if ($Activity['ActivityUserID'] == $Activity['NotifyUserID'] && !GetValue('Force', $Options)) {
         ...
         return; // don't notify users of something they did.
			...
			// Added for Post Scheduler plugin. Fire event before sending an email,
			// so that Plugin can prevent it if the Discussion to which the Activity
			// is associated is scheduled
			$this->EventArguments['Activity'] = &$Activity;
