 ...
 * - Added support for "longtext" fields.
 *
 * NOTES
...
if(class_exists('Gdn_MySQLStructure', FALSE)) {
	return;
}
...
class Gdn_MySQLStructure extends Gdn_DatabaseStructure {
   ...
   protected function _DefineColumn($Column) {
      if (!is_array($Column->Type) && !in_array($Column->Type, array('tinyint', 'smallint', 'mediumint', 'int', 'bigint', 'char', 'varchar', 'varbinary', 'date', 'datetime', 'mediumtext', 'text', 'longtext', 'decimal', 'float', 'double', 'enum', 'timestamp')))
...
\Aelia\OverridesManager::RegisterOverride('Gdn_MySQLStructure');
