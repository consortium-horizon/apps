...
/**
 * IMPORTANT - Modified version for Vanilla 2.0 (NOT COMPATIBLE with Vanilla 2.1+)
 * This file has been modified and distributed with Aelia Foundation Classes
 * plugin. The following changes have been made:
 * - Increased flexibility of permission checking in SideMenuModule::CheckPermissions().
 *   The new logic allows to specify if User should have ANY of the permissions,
 *   or ALL of them, by passing a "fullpermissionsmatch" attribute (bool).
 *
 * NOTES
 * - This file can be kept even if the Aelia Foundation Classes plugin is
 * disabled and/or uninstalled, as the changes won't have an impact on
 * Vanilla's normal operations.
 * - This file alone is distributed under GPL Licence, as per requirements in
 * original file. This licensing model doesn't apply to the package with which
 * this file was distributed, which will still be covered by its own Licence.
 *
   ...
   class SideMenuModule extends Gdn_Module {
      ...
      public function AssetTarget() {
         return 'Menu';
			...
			/**
			 * Determines if the permission to access a menu link should be checked on
			 * an "any of the specified permissions" or "all of the specified permissions".
			 * This will make permission checking more flexible, as it's hard coded
			 * to "full match" in standard Vanilla Core.
			 *
			 * @param array MenuItem An array describing a menu item.
			 * @return bool
			 */
			protected function FullPermissionMatch($MenuItem) {
				$ItemAttributes = GetValue('Attributes', $MenuItem);
				$FullPermissionMatch = GetValue('fullpermissionmatch', $ItemAttributes, true);

				return $FullPermissionMatch;
			}
...
\Aelia\OverridesManager::RegisterOverride('SideMenuModule');
